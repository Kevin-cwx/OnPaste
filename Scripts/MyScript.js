...
class ImageWindow {
    ...
    updateMagnifier(mouseX, mouseY) {
        ...
        try {
            mCtx.drawImage(
                this.image,
                imageX - halfSource, imageY - halfSource, sourceSize, sourceSize,
                0, 0, size, size
            );
        } catch (e) { }
    ...
    cropSelection() {
        ...
        croppedCtx.drawImage(this.image, cropX, cropY, cropW, cropH, 0, 0, cropW, cropH);
    ...
    getCroppedCanvas() {
        ...
        tCtx.drawImage(this.image, 0, 0, this.image.width, this.image.height, 0, 0, scaledWidth, scaledHeight);
        return tempCanvas;
    }
}
...
function renderCollection() {
    ...
    imageHistory.forEach((src) => {
        ...
        pasteBtn.innerHTML = "<i class='fa-solid fa-arrow-up-right-from-square'></i>";
...
function rebuildMenu() {
    ...
    menuItems.forEach((item) => {
        ...
        switch (item.text) {
            case "Copy": icon.className = "fa-solid fa-copy"; break;
            case "Paste": icon.className = "fa-solid fa-paste"; break;
            case "Download": icon.className = "fa-solid fa-download"; break;
            case "Refresh": icon.className = "fa-solid fa-arrows-rotate"; break;
            case "Crop": icon.className = "fa-solid fa-crop"; break;
            case "Reset Zoom": icon.className = "fa-solid fa-compress"; break;
        }
...
function showCustomContextMenu(x, y, hasImage) {
    ...
    children.forEach(child => {
        ...
        if ((text === "Copy" || text === "Download" || text === "Crop" || text === "Reset Zoom") && !hasImage) {
    ...
    switch (actionName) {
        ...
        case "reset":
            ...
            if (menuTargetWindow.image) {
                ...
                // Recenter
                menuTargetWindow.offsetX = (menuTargetWindow.canvas.width - menuTargetWindow.image.width) / 2;
                menuTargetWindow.offsetY = (menuTargetWindow.canvas.height - menuTargetWindow.image.height) / 2;
...
document.addEventListener("keydown", (e) => {
    ...
    if ((e.ctrlKey && e.key === 'y') || (e.ctrlKey && e.shiftKey && e.key === 'z')) { // Support both standard Redo shortcuts
        e.preventDefault();
        activeWindow.redo();
    }
});